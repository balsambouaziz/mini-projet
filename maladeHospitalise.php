...
while($res=mysqli_fetch_array($req)){
	...
	$infirmier=$res["idInfirmier"];

	$req1=mysqli_query($con,"select * from malade where id='{$malade}'");
	$req2=mysqli_query($con,"select * from medecin where id='{$medecin}'");
	$req3=mysqli_query($con,"select * from infirmier where id='{$infirmier}'");
	$res1=mysqli_fetch_array(req1);
	$res2=mysqli_fetch_array(req2);
	$res3=mysqli_fetch_array(req3);
echo '<tr><td>'.$res1["id"].' </td><td>'.$res1["nom"].' </td> <td>'.$res1["prenom"].' </td> <td> '.$res1["dateNaiss"].'</td> <td>'.$res2["nom"].'</td>  <td>'.$res3["nom"].'</td> <td>'.$res["chambre"].'</td><td>'.$res["dateEntree"].'</td> ';
if($res["dateSortie"]==null)
	echo '<td><button class="button">sortie</button></td> </tr>';
else echo '<td>'.date("d/m/y").'</td></tr>';
	}

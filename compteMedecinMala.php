...
<div class="topnav">
  ...
  <a  href="compteMedecin.php">mes informatons</a>
  <a class="active" href="compteMedecinMala.php">mes malades</a>
...
<?php
$idMedecin=$_SESSION["medecin"];

$con=mysqli_connect("localhost","root","","hopital");
$req1=mysqli_query($con,"select * from malade");
$req=mysqli_query($con,"select * from dossiermedical where idMedecin='{$idMedecin}'");
while($res1=mysqli_fetch_array($req1)){
	
	while($res=mysqli_fetch_array($req)){
	if ($res1["id"]==$res["idMalade"]){
	$malade=$res["idMalade"];
    echo '<tr><td><a href="dossierMedical.php?id='.$res1["id"].'">'.$res1["id"].'</a> </td><td>'.$res1["nom"].' </td><td>'.$res1["prenom"].' </td><td> '.$res1["dateNaiss"].'</td> <td>'.$res1["numtel"].'</td> <td>'.$res["daterendvous"].'</td></tr>';
	break;}
